  ...
  return (
    <div className="pointer-events-none absolute bottom-4 left-4 rounded-md border border-white/15 bg-black/70 p-1.5 backdrop-blur">
      <svg width={SIZE} height={SIZE} role="img" aria-label="City minimap">
        <rect x={0} y={0} width={SIZE} height={SIZE} fill="#111318" rx={4} />
        {/* Top-level communities only — nesting every folder would stack the
            map with rectangles too small to tell apart at 140px. */}
        {districts
          .filter((district) => district.level === 0)
          .map((district) => {
